...
function Save-SettingsFile {
    param(
        ...
        [bool]$IncludeWemCpuSpikeProtectionEvents = $false,
        [int]$WemPriorityLoweringSeconds = 180,
        [int]$WemEventContextCooldownMinutes = 10,
    ...
    $settings = [ordered]@{
        ...
        IncludeWemCpuSpikeProtectionEvents = $IncludeWemCpuSpikeProtectionEvents
        WemPriorityLoweringSeconds = $WemPriorityLoweringSeconds
        WemEventContextCooldownMinutes = $WemEventContextCooldownMinutes
...
function Load-GuiData {
    ...
    if ($runWemCpuSpikeProtectionBox) { $runWemCpuSpikeProtectionBox.Checked = [bool](Get-SettingValue -Settings $settings -Name 'IncludeWemCpuSpikeProtectionEvents' -DefaultValue $false) }
    ...
    if ($taskWemCpuSpikeProtectionBox) { $taskWemCpuSpikeProtectionBox.Checked = [bool](Get-SettingValue -Settings $settings -Name 'IncludeWemCpuSpikeProtectionEvents' -DefaultValue $false) }
...
function Save-GuiData {
    ...
    Save-SettingsFile -CpuSampleSeconds ([int]$runCpuSampleBox.Value) `
        ...
        -IncludeWemCpuSpikeProtectionEvents $runWemCpuSpikeProtectionBox.Checked `
...
function Start-HealthCheckRun {
    ...
    $arguments = @(
        ...
        '-MaxConcurrentDefenderPerfRecordings', ([int]$runDefenderConcurrentBox.Value),
        '-WemTriggerServerCpuPercent', ([double]$runWemTriggerBox.Value),
        '-WemPriorityLoweringSeconds', ([int](Get-SettingValue -Settings (Read-SettingsFile) -Name 'WemPriorityLoweringSeconds' -DefaultValue 180)),
        '-WemEventContextCooldownMinutes', ([int](Get-SettingValue -Settings (Read-SettingsFile) -Name 'WemEventContextCooldownMinutes' -DefaultValue 10)),
        '-WemEventWindowMinutes', ([int]$runWemWindowBox.Value),
        '-WemLogTailLines', ([int]$runWemTailLinesBox.Value)
    )
    ...
    if ($runWemCpuSpikeProtectionBox.Checked) { $arguments += '-IncludeWemCpuSpikeProtectionEvents' }
...
function Register-HealthCheckScheduledTask {
    ...
    if ($taskWemCpuSpikeProtectionBox.Checked) { $actionArguments += '-IncludeWemCpuSpikeProtectionEvents' }
    ...
    $credential = Get-Credential -UserName $principalUser -Message 'Bitte Kennwort fuer den geplanten HealthCheck-Task eingeben. Das Kennwort wird im Windows Task Scheduler gespeichert, damit "Do not store password" nicht gesetzt wird.'
    if (-not $credential) { throw 'Taskplanung abgebrochen: Es wurden keine Anmeldeinformationen angegeben.' }
    $principalUser = $credential.UserName
    $principal = New-ScheduledTaskPrincipal -UserId $principalUser -LogonType Password -RunLevel Highest
    ...
    try {
        ...
        Register-ScheduledTask -TaskName $taskName -TaskPath $taskPath -InputObject $taskDefinition -User $principalUser -Password $plainPassword -Force | Out-Null
    }
    finally {
        if ($passwordBstr -ne [IntPtr]::Zero) { [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($passwordBstr) }
        $plainPassword = $null
    }
    $successMessage = "Scheduled Task eingerichtet: $taskPath$taskName`r`nAusfuehrung: unabhaengig von Benutzeranmeldung, Kennwort gespeichert (Do not store password ist deaktiviert)"
...
function Complete-HealthCheckRun {
    ...
    try {
        ...
        if (-not $script:HealthCheckProcess.HasExited) { $script:HealthCheckProcess.WaitForExit(1000) | Out-Null }
        $script:HealthCheckProcess.Refresh()
        $exitCode = [int]$script:HealthCheckProcess.ExitCode
    }
    catch {
        $exitCode = -1
        Add-StatusLine "ExitCode konnte nicht gelesen werden: $($_.Exception.Message)"
    }
...
$runWemCpuSpikeProtectionBox = New-Object System.Windows.Forms.CheckBox
$runWemCpuSpikeProtectionBox.Text = 'CPU Spike Protection aktiv'
$runWemCpuSpikeProtectionBox.Location = New-Object System.Drawing.Point(610, 535)
$runWemCpuSpikeProtectionBox.Size = New-Object System.Drawing.Size(240, 24)
$runTab.Controls.Add($runWemCpuSpikeProtectionBox)
...
$taskWemCpuSpikeProtectionBox = New-Object System.Windows.Forms.CheckBox
$taskWemCpuSpikeProtectionBox.Text = 'CPU Spike Protection aktiv'
$taskWemCpuSpikeProtectionBox.Location = New-Object System.Drawing.Point(610, 710)
$taskWemCpuSpikeProtectionBox.Size = New-Object System.Drawing.Size(240, 24)
$taskTab.Controls.Add($taskWemCpuSpikeProtectionBox)
