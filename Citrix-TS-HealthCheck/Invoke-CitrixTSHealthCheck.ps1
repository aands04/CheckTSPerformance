...
try {
    ...
    while ($true) {
        if (($round + 1) -gt $maxRounds) { $endReason = 'MaxRoundsReached'; Write-RunLog -Path $runLogFile -Message "Keine neue Runde: MaxRounds erreicht ($maxRounds)."; break }
        ...
        $remaining = [int][Math]::Floor(($hardEndTime - (Get-Date)).TotalSeconds)
